document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================
     THEME — light/dark, persisted, animated via View Transitions
     (circular reveal from the toggle button's position; falls
     back to an instant swap on browsers without support)
  ========================================================== */
  ...
  paintTheme(theme); // initial paint, no animation
  ...
  /* ==========================================================
     PERSONA TOGGLE — swaps text + hero icons in place with a
     wash transition (separate from the theme's circular reveal)
  ========================================================== */
  ...
  /* ==========================================================
     CURSOR — smooth spring-follow dot that morphs to "fill" the
     exact size/shape of small controls on hover (mix-blend-mode
     keeps text underneath legible instead of hiding it)
  ========================================================== */
  const cursorDot = document.getElementById('cursorDot');
  const finePointer = window.matchMedia('(pointer:fine)').matches;
  const FILL_SELECTOR = '.btn, .dock-item, .p-btn, .icon-btn, .kbd-chip, .clink, .p-opt, .copy-btn, .back-to-top, .cmdk-item';
  ...
  if (!reduced && finePointer) {
    ...
    function fillTarget(el) {
      ...
      cursorDot.style.transition = 'left .22s var(--ease-spring), top .22s var(--ease-spring), width .22s var(--ease-spring), height .22s var(--ease-spring), border-radius .22s ease';
    ...
    function releaseFill() {
      filling = false;
      cursorDot.style.transition = 'width .3s var(--ease-spring), height .3s var(--ease-spring), border-radius .3s ease';
      cursorDot.style.width = '8px';
      cursorDot.style.height = '8px';
      cursorDot.style.borderRadius = '50%';
      cursorDot.style.transform = 'translate(-50%,-50%)';
    }
    document.querySelectorAll(FILL_SELECTOR).forEach(el => {
      el.addEventListener('mouseenter', () => fillTarget(el));
      el.addEventListener('mouseleave', releaseFill);
    });

    /* magnetic pull for the same controls — a gentle nudge toward the pointer */
  ...
  /* ==========================================================
     AMBIENT PIXEL-STAR BACKGROUND — drifts AND twinkles, so the
     page never looks static. Monochrome, theme-aware, cheap.
  ========================================================== */
  let starBurstUntil = 0; // used by the easter egg below
  (function initPixelStars() {
    ...
    const ACCENT_LIGHT = '0,112,243';
    const ACCENT_DARK = '50,145,255';
    let t = 0;
    function loop() {
      t += 0.02;
      ctx.clearRect(0, 0, w, h);
      const dark = isDarkMode();
      const burst = performance.now() < starBurstUntil;
      stars.forEach(s => {
        // gentle drift, wrapping at the edges
        s.x += s.vx; s.y += s.vy;
        if (s.x < 0) s.x = w; if (s.x > w) s.x = 0;
        if (s.y < 0) s.y = h; if (s.y > h) s.y = 0;

        const twinkle = 0.5 + 0.5 * Math.sin(t * s.speed + s.phase);
        const alpha = burst ? 0.35 + 0.45 * twinkle : 0.12 + 0.32 * twinkle;
        const rgb = burst ? (dark ? ACCENT_DARK : ACCENT_LIGHT) : (dark ? '255,255,255' : '0,0,0');
        ctx.fillStyle = `rgba(${rgb},${burst ? alpha : alpha * (dark ? 1 : 0.75)})`;
        ctx.fillRect(s.x, s.y, s.size, s.size);
      });
  ...
  console.log('%cI\'m Shubham — I built this whole site by hand, no template. If you\'re hiring for data or business analyst roles, my email is in the footer. Also: try the Konami code.', 'font-size:12px;color:#888;');

  const KONAMI = ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft', 'ArrowRight', 'b', 'a'];
  let konamiIndex = 0;
  document.addEventListener('keydown', (e) => {
    if (cmdkOverlay && cmdkOverlay.classList.contains('is-open')) return;
    const key = e.key.length === 1 ? e.key.toLowerCase() : e.key;
    if (key === KONAMI[konamiIndex]) {
      konamiIndex++;
      if (konamiIndex === KONAMI.length) {
        konamiIndex = 0;
        starBurstUntil = performance.now() + 2400;
        showToast('🔓 Achievement unlocked: you found the Konami code');
      }
    } else {
