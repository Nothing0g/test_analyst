document.addEventListener('DOMContentLoaded', () => {
  ...
  const CASES = [
    {
      ...
      status: 'Resolved',
      report: {
        legend: [{ label: 'Ride demand, by hour', color: 'var(--rc-accent)' }],
        chartType: 'line',
    ...
    },
    {
      ...
      status: 'Resolved',
      report: {
        legend: [{ label: 'Logistic Regression', color: 'var(--rc-accent)' }, { label: 'Random Forest', color: 'var(--rc-accent2)' }],
        chartType: 'bars',
        chartBars: [
          { label: 'Recall — Logistic Regression', value: 56, color: 'var(--rc-accent)' },
          { label: 'Recall — Random Forest', value: 21, color: 'var(--rc-accent2)' }
        ],
        stats: [
  ...
  function renderChart(report) {
    ...
    if (report.chartType === 'line') {
      ...
      bars.forEach((b, i) => {
        ...
        label.setAttribute('fill', '#f5f5f7');
  ...
  /* ==========================================================
     CURSOR — transform-only physics. Position AND size are both
     driven by a single `transform: translate() scale()` string,
     computed from a fixed 8x8 base box (transform-origin: 0 0).
     No left/top/width/height writes, and target measurements are
     cached once per mouseenter instead of re-read every frame —
     this was previously the most expensive thing on the page,
     running a layout write *and* a style read on every mousemove.
  ========================================================== */
  ...
  const BASE = 8; // cursorDot's fixed CSS width/height in px

  if (!reduced && finePointer) {
    let mouseX = window.innerWidth / 2, mouseY = window.innerHeight / 2;
    // current (lerped) and target state, all in "translate x/y, scale, radius" terms
    let cur = { x: mouseX - BASE / 2, y: mouseY - BASE / 2, sx: 1, sy: 1, r: BASE / 2 };
    let target = { x: cur.x, y: cur.y, sx: 1, sy: 1, r: BASE / 2 };
    let fillEl = null;
    ...
    function measureTarget(el) {
      ...
      target = { x: r.left, y: r.top, sx: r.width / BASE, sy: r.height / BASE, r: radius };
    }
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      if (!fillEl) {
        target = { x: mouseX - BASE / 2, y: mouseY - BASE / 2, sx: 1, sy: 1, r: BASE / 2 };
      }
    });
    ...
    function loop() {
      ...
      cursorDot.style.transform = `translate(${cur.x}px, ${cur.y}px) scale(${cur.sx}, ${cur.sy})`;
      cursorDot.style.borderRadius = cur.r + 'px'; // paint-only, not a layout property
