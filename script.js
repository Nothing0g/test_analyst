document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================
     CURSOR — pure physics: one rAF loop lerps current x/y/w/h/r
     toward a target every frame. No CSS transitions involved,
     so there's nothing to fight or rubber-band near edges.
  ========================================================== */
  ...
  if (!reduced && finePointer) {
    ...
    let cur = { x: mouseX - 4, y: mouseY - 4, w: 8, h: 8, r: 4 };
    let fillEl = null;

    window.addEventListener('mousemove', (e) => {
      cursorDot.style.opacity = '1';
      mouseX = e.clientX; mouseY = e.clientY;
      fillEl = e.target.closest ? e.target.closest(FILL_SELECTOR) : null;
    });

    function getTarget() {
      if (fillEl && document.body.contains(fillEl)) {
      ...
      return { x: mouseX - 4, y: mouseY - 4, w: 8, h: 8, r: 4 };
    }
